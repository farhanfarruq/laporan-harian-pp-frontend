...
const ReportDetailModal = ({ isOpen, onClose, report }) => {
    const { bidangList, jobdeskData } = useAppContext();

    if (!isOpen || !report) return null;

    const bidangName = bidangList.find(b => b.id === report.bidang)?.name || report.bidang;
    let jobdeskKey = report.bidang === 'bapakamar' ? `bapakamar_${report.tab || 'malam'}` : report.bidang;
    ...
    allJobdesks.forEach((job, index) => {
        if (report.jobdesk[index]) {
            ...
            const details = report.incomplete[index] || { alasan: '-', solusi: '-' };
            incompletedTasks.push({ job, ...details });
        }
    });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 mb-6 text-sm">
                    <div className="flex items-center"><FontAwesomeIcon icon={faUser} className="text-blue-500 mr-3 w-4" /> <strong>Pengurus:</strong><span className="ml-2 text-gray-700">{report.pengurus}</span></div>
                    <div className="flex items-center"><FontAwesomeIcon icon={faBriefcase} className="text-blue-500 mr-3 w-4" /> <strong>Bidang:</strong><span className="ml-2 text-gray-700">{bidangName}</span></div>
                    ...
                    <div className="flex items-center"><FontAwesomeIcon icon={faClock} className="text-blue-500 mr-3 w-4" /> <strong>Waktu Submit:</strong><span className="ml-2 text-gray-700">{dayjs(report.timestamp).format('HH:mm')}</span></div>
                </div>
                        ...
                        {incompletedTasks.length > 0 ? (
                             <div className="space-y-4">
                                {incompletedTasks.map((task, index) => (
                                     <div key={index} className="p-3 border border-red-200 rounded-lg bg-red-50 text-sm">
                                        <div className="flex items-start font-semibold">
                                            <FontAwesomeIcon icon={faTimesCircle} className="text-red-500 mr-3 mt-1" />
                                            <p className="text-red-800">{task.job}</p>
                                        </div>
                                        <div className="pl-7 mt-2 space-y-1 text-xs">
                                            <p><strong>Alasan:</strong> <span className="text-gray-600">{task.alasan}</span></p>
                                            <p><strong>Solusi:</strong> <span className="text-gray-600">{task.solusi}</span></p>
                                        </div>
                                     </div>
                                ))}
                             </div>
